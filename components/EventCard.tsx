  ...
  return (
    <div className="glass-card p-6 sm:p-8 flex gap-6 items-start">
      ...
      <div className="text-center min-w-[70px] py-3 rounded-xl bg-bg-deep/50 border border-gold/8">
        <div className="text-gold/60 text-xs uppercase tracking-[3px] mb-1">{month}</div>
        <div className="text-3xl font-serif font-light text-text-bright leading-none">{day}</div>
      </div>

      <div className="flex-1">
        <h3 className="font-serif text-xl text-text-bright font-light mb-3">{event.name}</h3>
        ...
        {event.dressCode && (
          <span className="inline-block text-xs tracking-[2px] uppercase px-3 py-1 rounded-full border border-gold/10 text-gold/60">
            {event.dressCode}
          </span>
        )}

        {event.description && (
          <p className="text-text-muted text-sm mt-3 leading-relaxed">{event.description}</p>
        )}
      </div>
    </div>
  );
